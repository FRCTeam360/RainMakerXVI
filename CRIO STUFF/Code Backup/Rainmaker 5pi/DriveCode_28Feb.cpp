...
public:
	...
	void shootSpeedChange(void) {                                          //adjusts the speed of the wheels on the shooter. launching only; picking up is a set speed
		...
		if(gamePad->GetRawButton(plusten) && plusten_pressed == false){    //checks to see if the INCREASE BY 10 button is pressed and if it is not already being pressed
			shooterspeed = shooterspeed + .1;                              //increases shooter speed by 10 percent
			plusten_pressed = true;                                        //indicates the button is being pressed so that it will not add 10 indefinitely
		}
		...
		if(gamePad->GetRawButton(minusten) && minusten_pressed == false){  //checks to see if the DECREASE BY 10 button is pressed
		...
		if(!gamePad -> GetRawButton(plusten)){                             //if INCREASE BY 10 button is no longer being pressed
			plusten_pressed = false;                                       //allow the add 10% function to run again
		}
		...
		if(!gamePad -> GetRawButton(minusten)){                            //ditto
	...
	void shooterControl(void){
			...
			if(shooterMode > 0.95){
				victorShooterFront = -0.35;
				victorShooterBack = -0.35;
				shootAllowed = false;
			}
	...
	{	
		...
		shootSpeedChange();
		/*
		if((leftStick->GetRawButton(softBall) || rightStick->GetRawButton(softBall)) && softBallPressed == false){
			softBallPressed = true;
			shallowAngle = !shallowAngle;
		}
		
		if(!(leftStick->GetRawButton(softBall) || rightStick->GetRawButton(softBall)) && softBallPressed == true){
	...
	void AutonomousPeriodic(void)
	{
		...
		if(autoStage == 1){
			...
			doPID(3, 0.0, 0.15, 0.015, armPosition->GetValue(), 610); // p, i, d, coefficients; dt; sensor input; target value
		...
		if(autoStage == 3){
			...
			doPID(3, 0.0, 0.15, 0.01, armPosition->GetValue(), 460);
