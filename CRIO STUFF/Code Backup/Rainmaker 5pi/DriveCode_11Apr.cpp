...
static const double encodingRatio = 1.73;
float rawCounts = 0;
...
{ 
	...
	Countdown *launchTimer;
	Countdown *autoTimer;
	
	EnhancedButton *angleUp;
...
public:
	...
	DriveControl(void)
	{
		...
		lights = new Relay(2, Relay::kForwardOnly);
		...
		launchTimer = new Countdown();
		autoTimer = new Countdown();
		
		angleUp = new EnhancedButton();
	...
	void armRotation(void)                                                  //controls the position/angle of the arm
	{
		...
		if(override < 0.95){                                                //if the arm power override is not pressed
			...
			if ((gamepad_axis3 > 0.1 || gamepad_axis3 < -0.1) && (armPosition -> GetValue() > 185 && armPosition -> GetValue() < 735)){               //checks if the analog control axis is out of the buffer zone
	...
	void shooterControl(void){
			...
			if(shooterMode > 0.95){
				victorShooterFront = 0.4;
				victorShooterBack = 0.4;
				shootAllowed = false;
			}
	...
	void changeGear(void){
		...
		if(highGear == true){
			...
			console->Printf(DriverStationLCD::kUser_Line6, 1, "High Gear");
		}
		...
		if(highGear == false){
			...
			console->Printf(DriverStationLCD::kUser_Line6, 1, "Low Gear");
		}
	}
	...
	void distanceCalculator(int inches){
		rawCounts = ((inches / wheel_circ) * encodingRatio) * 500;   //920 counts on the practice bot
	}
	
	
	void lightControl(void){
		if(leftStick->GetRawButton(toggleLights) || rightStick->GetRawButton(toggleLights)){
			lightsPressed = true;
		}
		
		if(!(leftStick->GetRawButton(toggleLights) || rightStick->GetRawButton(toggleLights)) && lightsPressed == true){
			lightsPressed = false;
			lightsOn = !lightsOn;
			console -> Clear();
		}
		...
		if(lightsOn == true){
			lights -> Set(Relay::kOn);
			console->Printf(DriverStationLCD::kUser_Line5, 1, "Lights On");
		}
		
		if(lightsOn == false){
			lights -> Set(Relay::kOff);
			console->Printf(DriverStationLCD::kUser_Line5, 1, "Lights Off");
		}
	}
	...
	void AutonomousInit(void)
	{
		...
		autoStage = 0;
		distanceCalculator(153);
		
		leftEncoder -> Start();
		leftEncoder -> Reset();
		
		shooterfront -> Set(-0.45);
		shooterback -> Set(-0.45);
	...
	void AutonomousPeriodic(void)
	{
		...
		console->Printf(DriverStationLCD::kUser_Line5, 1, "Travelled: %d", leftCount);
		console->Printf(DriverStationLCD::kUser_Line6, 1, "Target: %5.0f", rawCounts);
		console->Printf(DriverStationLCD::kUser_Line3, 1, "Angle: %d", armPosition -> GetValue());
		console -> UpdateLCD();
				
		if(autoStage == 0 && leftCount < 7200){
			drive_right -> Set(1);
			drive_left -> Set(-1);
			
			doPID(0.01, 0, 0.0015, 0.01, armPosition -> GetValue(), 563);
		...
		*/
		...
		if(autoStage == 1){
			doPID(0.03, 0, 0.0015, 0.01, armPosition -> GetValue(), 563);
