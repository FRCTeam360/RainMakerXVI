...
//convenient to assign your buttons through variables so that you need only change the values of one thing to completely remap your controller
...
static const int plusten = 8;
static const int plusone = 6;
static const int minusten = 7;
...
//ditto (see comments in JOYSTICK section)
bool plusten_pressed = false;
bool plusone_pressed = false;
bool minusten_pressed = false;
...
static const int armShootLeft = 345;     //read only. value of arm potentiometer when shooting to the left
static const int armShootRight = 590;    //ditto except to the right
...
float shooterspeed = 0;                  //used in void speedChange() and void shooterControl to control the speed of the shooter wheels
...
public: DriveControl(void){
	
		UINT32 comprelaychannel = 1;
		UINT32 pressureswitchchannel = 6;
		
		drive_right = new Jaguar(1, 2); //right side of drive train. parameters - 9403 module #; Digital Sidecar port
		drive_left = new Jaguar(1, 1);
		arm = new Jaguar(1, 3);
		shooterfront = new Jaguar(1, 4); 
		shooterback = new Jaguar(1, 5);
		...
		sole[1] = new DoubleSolenoid(1, 1, 2); //launcher solenoid. parameters - 9472 module #; port; port (two ports needed for double actions)
		sole[2] = new DoubleSolenoid(1, 3, 4); //shifter solenoid
	...
	void armRotation(void){                                                  //controls the position/angle of the arm
		...
		console->Printf(DriverStationLCD::kUser_Line4, 1, "Correction: %d\n", angleCorrection);  //prints the value of our angle correction to the driver station
	...
	void shootSpeedChange(void) {
		
		if(gamePad->GetRawButton(plusten) && plusten_pressed == false){
			shooterspeed = shooterspeed + .1;
			plusten_pressed = true;
		}
		
		if(gamePad->GetRawButton(plusone) && plusone_pressed == false){
			shooterspeed = shooterspeed + .01;
			plusone_pressed = true;
		}
		
		if(gamePad->GetRawButton(minusten) && minusten_pressed == false){
			shooterspeed = shooterspeed - .1;
			minusten_pressed = true;
		}
		
		if(gamePad->GetRawButton(minusone) && minusone_pressed == false){
			shooterspeed = shooterspeed - .01;
		...
		if(!gamePad -> GetRawButton(plusten)){
			plusten_pressed = false;
		}
		
		if(!gamePad -> GetRawButton(plusone)){
			plusone_pressed = false;
		}
		
		if(!gamePad -> GetRawButton(minusten)){
			minusten_pressed = false;
		}
		
		if(!gamePad -> GetRawButton(minusone)){
	...
	void shooterControl(void){
			...
			if(shooterMode > 0.95){
				victorShooterFront = -0.40;
				victorShooterBack = -0.40;
				shootAllowed = false;
			}
			...
			if(shootAllowed == true){
				...
				if(gamePad -> GetRawButton(ShootSole)){
					sole[1]->Set(DoubleSolenoid::kForward);
	...
	void changeGear(void){
		...
		if(!(leftStick -> GetRawButton(2) || rightStick -> GetRawButton(2)) && shifterPressed == true){
			shifterPressed = false;
			highGear = !highGear;
		}
		
		if(highGear == true){
			sole[2]->Set(DoubleSolenoid::kForward);
		}
		
		if(highGear == false){
			sole[2]->Set(DoubleSolenoid::kReverse);
		}
	}
	...
	void encoderTest(void){
		...
		console->Printf(DriverStationLCD::kUser_Line4, 1, "Left Encoder: %d\n", leftCount);
	}
	
	
	void armAngle(void){
		console->Printf(DriverStationLCD::kUser_Line3, 1, "Arm Angle: %d\n", armPosition -> GetValue());
	}
	...
	void AutonomousPeriodic(void){
		...
		if(autoStage == 1){
			...
			doPID(3, 0.0, 0.15, 0.01, armPosition->GetValue(), 305); // p, i, d, coefficients; dt; sensor input; target value
			...
			if((current_time - initial_time) > 4.5){
				sole[1]->Set(DoubleSolenoid::kForward);
				autoStage = 2;
				initial_time = GetTime();
			}
		...
		if(autoStage == 3){
			...
			doPID(3, 0.0, 0.15, 0.01, armPosition->GetValue(), 470);
	...
	void TeleopPeriodic(void){
		...
		console->UpdateLCD();
		//printf("%d\n", leftEncoder -> Get(), encoder_two -> Get());
	}
};
