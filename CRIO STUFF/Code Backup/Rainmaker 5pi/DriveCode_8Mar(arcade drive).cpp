...
static const int shifter = 5;
static const int softBall = 11;
static const int cancelTurn = 2;
static const int cancelDrive = 1;
...
static const int armShootRight = 320;     //read only. value of arm potentiometer when shooting to the left
static const int armShootLeft = 600;    //ditto except to the right
...
long leftCount = 0;                      //left drive encoder count. encoder count will never be negative hence why it is a long class integer
long rightCount = 0;
...
float shooterspeed = 0.5;                //used in void speedChange() and void shooterControl to control the speed of the shooter wheels
...
static const float wheel_circ = (6 *(PI));	 //wheel measurments are in inches, and include tread thickness, of 1/4in per side
static const float encodingRatio = 26/15;
//float encoding_ratio = (7/5);			 //encoder is direct mount on transmission output, this ratio is from the wheels to output
float rawCounts = 0;
//end Distance Calculator variables
...
{ 
	...
	Joystick *driveStick;//set joysticks
	Joystick *gamePad;
...
public:
	...
	DriveControl(void)
	{
		...
		rightEncoder = new Encoder (1,2, true);
	...
	void RCdrive(void){                                                     //non-standard drive train control. this is similar to arcade drive. splits it into two sticks - throttle and steering
		driveY = driveStick -> GetY();                                  //updating joystick states. right stick is throttle & direction
		driveX = driveStick -> GetX();                                     //left stick is the steering
							 
		victorRight_speed = 0;                                              //stops the motors if no input is detected later on
		victorLeft_speed = 0; 
		
		if(driveStick -> GetRawButton(cancelTurn)){
			driveX = 0;
		}
		
		if(driveStick -> GetRawButton(cancelDrive)){
			driveY = 0;
		}
		
		if(driveY > 0.1 || driveY < -0.1){                          //checks to see if the throttle and direction joystick is out of the buffer zone
			
			if(driveX > 0.1){                                            //if the left joystick is pushed to the right
				victorLeft_speed = driveY;                              //left speed is unimpeded - set directly to the throttle&direction indicated by the right stick
				victorRight_speed = driveY * (1 - (driveX * 0.6));           //right speed is scaled back proportional to the deflection of the left joystick
			}                                                               //long story short this checks to see if we want a right turn and then makes robot do a sweeping right turn
			
			if(driveX < -0.1){                                           //if we want left turn - ditto. see previous comments
				victorLeft_speed = driveY * (1 + (driveX * 0.6));
				victorRight_speed = driveY;
			}
			
			if(!(driveX > 0.1 || driveX < -0.1)){                     //if we want no turn
				victorLeft_speed = driveY;                              //just go straight. set both directly to throttle and direction
				victorRight_speed = driveY;
			}
		}
		
		if(!(driveY > 0.1 || driveY < -0.1) && (driveX > 0.1 || driveX < -0.1)){  //if throttle is at zero but we are still trying to steer
			
			if(driveX > 0.1){                                            //check to see which way
				victorLeft_speed = -driveX;                              //pivot in place
				victorRight_speed = driveX;
			}
			
			if(driveX < -0.1){
				victorLeft_speed = -driveX;
				victorRight_speed = driveX;
			}
		}
	...
	void shooterControl(void){
			...
			if(shootAllowed == false){
				...
				if(gamePad -> GetRawButton(ShootSole)){
					console->Printf(DriverStationLCD::kUser_Line6, 1, "Im sorry Dave");
					initial_time = GetTime();
				}
				...
				if((current_time - initial_time) > 0.7){
					console->Printf(DriverStationLCD::kUser_Line6, 1, "----------------");
				}
			}
		}
	...
	void changeGear(void){
		...
		if(driveStick -> GetRawButton(shifter)){
			shifterPressed = true;
		}
		
		if(!(driveStick -> GetRawButton(shifter)) && shifterPressed == true){
	...
	void encoderTest(void){
		...
		rightCount = rightEncoder ->GetRaw();
		
		console->Printf(DriverStationLCD::kUser_Line5, 1, "Right Encoder: %d", rightCount);
		//console->Printf(DriverStationLCD::kUser_Line5, 1, "Left Encoder: %d", leftCount);
	}
	...
	void distanceCalculator(int inches){
		rawCounts = ((inches / wheel_circ) * encodingRatio) * 920;
	}
	...
	{	
		...
		shootSpeedChange();
		/*
		if((leftStick->GetRawButton(softBall) || driveStick->GetRawButton(softBall)) && softBallPressed == false){
			softBallPressed = true;
			shallowAngle = !shallowAngle;
		}
		
		if(!(leftStick->GetRawButton(softBall) || driveStick->GetRawButton(softBall)) && softBallPressed == true){
			softBallPressed = false;
		}
		
		if(shallowAngle == true){
			angleCorrection = -8;
			console->Printf(DriverStationLCD::kUser_Line6, 1, "Soft Ball");
		}
		
		if(shallowAngle == false){
			angleCorrection = 0;
			console->Printf(DriverStationLCD::kUser_Line6, 1, "Hard Ball");
		}
		*/
		console -> UpdateLCD();
		//encoder_two->Stop();
	...
	void AutonomousPeriodic(void)
	{
		...
		if(autoStage == 3){
			...
			doPID(3, 0.0, 0.15, 0.015, armPosition->GetValue(), 330); // p, i, d, coefficients; dt; sensor input; target value
			...
			console->Printf(DriverStationLCD::kUser_Line4, 1, "Arm Angle: %d\n", armPosition -> GetValue());
		...
		if(autoStage == 2){
			...
			doPID(3, 0.0, 0.15, 0.01, armPosition->GetValue(), 330);
			...
			if((current_time - initial_time) > 0.45){
				autoStage = 3;
